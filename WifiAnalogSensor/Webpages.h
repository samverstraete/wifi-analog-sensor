...
const char WEBPAGES_ADMIN[] PROGMEM = R"=====(
	<form id='form' action='/config' method='post'><br>
		<div class='msg'>
			<label for='tname'>Thing name</label><input type='text' id='tname' name='tname' maxlength=32 value='{tname}' /><br>
		...
		<div class='msg'>
			<label for='url'>URL to call</label><input type='text' id='url' name='url' maxlength=199 value='{url}' /><br>
			<label for='sync'>Seconds between URL calls</label><input type='number' id='sync' name='sync' maxlength=4 value='{sync}' min='1' max='99999' step='1'/><br>
			<label for='shot'>Milliseconds between samples</label><input type='number' id='shot' name='shot' maxlength=4 value='{shot}' min='1' max='99999' step='1'/><br>
		</div><br>
...
public:
	...
	void serveAdmin(ESP8266WebServer* webServer, bool* needsRestart);
	void handleNotFound(ESP8266WebServer* webServer);
	static boolean isIp(String str);
	static String toStringIp(IPAddress ip);
	boolean handleCaptivePortal(ESP8266WebServer* webServer);
